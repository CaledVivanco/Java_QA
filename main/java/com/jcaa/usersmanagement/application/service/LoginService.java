...
public final class LoginService implements LoginUseCase {
    ...
    public UserModel execute(final LoginCommand command) {
        ...
        final UserModel user = findUser(command.email());
    ...
    private UserModel findUser(String emailValue) {
        return getUserByEmailPort
                .getByEmail(new UserEmail(emailValue))
    ...
    private void validatePassword(UserModel user, String password) {
        if (!user.getPassword().verifyPlain(password)) {
            throw InvalidCredentialsException.becauseCredentialsAreInvalid();
        }
    }

    private void validateUserStatus(UserModel user) {
        if (user.getStatus() != UserStatus.ACTIVE) {
            throw InvalidCredentialsException.becauseUserIsNotActive();
        }
    }
