...
public final class UpdateUserService implements UpdateUserUseCase {
    ...
    public UserModel execute(final UpdateUserCommand command) {
        ...
        final UserModel currentUser = findUser(userId);

        validateEmail(command.email(), userId);

        final UserModel updated = mapper.toUpdatedModel(command, currentUser);

        final UserModel saved = updateUserPort.update(updated);

        emailNotificationService.notifyUserUpdated(saved);

        return saved;
    }

    private UserModel findUser(UserId userId) {
        return getUserByIdPort
                .getById(userId)
                .orElseThrow(() -> UserNotFoundException.becauseIdWasNotFound(userId.value()));
    }

    private void validateEmail(String emailValue, UserId userId) {
        final UserEmail email = new UserEmail(emailValue);

        getUserByEmailPort.getByEmail(email).ifPresent(user -> {
